...
GENSKILLS_MARKER = "The user invoked /genskills"
GENSKILLS_LAUNCHER = {
    "id": "genskills",
    "label": "genSkills",
    "icon": "🧩",
    "prefix": "/genskills",
    "description": "把目标保存成可复用做法，并可继续执行",
}
...
def _text_for_slash_detection(content: Any) -> str:
    if isinstance(content, str):
        return content
    return _normalize_chat_content(content)


def _parse_genskills_goal(content: Any) -> Optional[str]:
    text = _text_for_slash_detection(content).strip()
    if not text:
        return None
    if text.startswith("／"):
        text = "/" + text[1:]
    match = re.match(r"^/(?:genskills|genskill)\b(?:\s+(.*))?$", text, re.IGNORECASE | re.DOTALL)
    if not match:
        return None
    return (match.group(1) or "").strip()


def _extract_legacy_genskills_goal(text: str) -> Optional[str]:
    if GENSKILLS_MARKER not in text:
        return None
    match = re.search(r"用户想保存的做法[:：]\s*(.+)", text)
    if match:
        return match.group(1).strip()
    return ""


def _has_genskills_context(messages: List[Dict[str, Any]]) -> bool:
    for msg in messages:
        if msg.get("role") != "user":
            continue
        content = msg.get("content", "")
        text = _text_for_slash_detection(content)
        if GENSKILLS_MARKER in text or _parse_genskills_goal(content) is not None:
            return True
    return False


def _rewrite_genskills_messages(
    ...
    *,
    ...
    for msg in messages:
        if msg.get("role") != "user":
            ...
            continue
        content = msg.get("content", "")
        goal = _parse_genskills_goal(content)
        if goal is None:
            text = _text_for_slash_detection(content)
            goal = _extract_legacy_genskills_goal(text)
            if goal is None:
                rewritten.append(msg)
                continue
        try:
            from hermes_cli.genskills import build_genskills_invocation_message
            content = build_genskills_invocation_message(goal, task_id=session_id)
        except Exception:
            logger.exception("Failed to build /genskills invocation message")
        active = True
        rewritten.append({**msg, "content": content})
    return rewritten, active
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def _create_agent(
        ...
        force_toolsets: Optional[set[str]] = None,
    ) -> Any:
        """
        ...
        if force_toolsets is not None:
            enabled_toolsets_set = set(force_toolsets)
        else:
            enabled_toolsets_set = set(_get_platform_tools(user_config, "api_server"))
        if extra_toolsets and force_toolsets is None:
            enabled_toolsets_set.update(extra_toolsets)
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        combined_messages = history + [{"role": "user", "content": user_message}]
        combined_messages, genskills_active = _rewrite_genskills_messages(
            combined_messages,
            session_id=session_id,
        )
        if combined_messages:
            user_message = combined_messages[-1].get("content", "")
            history = combined_messages[:-1]
        if not genskills_active:
            genskills_active = _has_genskills_context(history)
        extra_toolsets = {"skills"} if genskills_active else None
        force_toolsets = {"skills"} if genskills_active else None
