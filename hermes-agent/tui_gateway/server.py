...
def _make_agent(
    ...
    return AIAgent(
        ...
        enabled_toolsets=enabled_toolsets if enabled_toolsets is not None else _load_enabled_toolsets(),
        platform="tui",
    ...
    if name == "genskills":
        ...
        if session.get("running"):
            return _err(
                rid,
                4009,
                "session busy — /interrupt the current turn before /genskills",
            )
        try:
            from hermes_cli.genskills import (
                build_genskills_invocation_message,
                genskills_title,
            )
        except Exception as exc:
            return _err(rid, 5030, f"genskills unavailable: {exc}")

        old_key = session.get("session_key") or ""
        new_key = _new_session_key()
        title = genskills_title(arg)
        db = _get_db()
        if db is not None:
            try:
                if old_key:
                    db.end_session(old_key, "genskills")
                db.create_session(
                    new_key,
                    source="tui",
                    model=_resolve_model(),
                    parent_session_id=old_key or None,
                    cwd=_session_cwd(session),
                )
                db.set_session_title(new_key, title)
            except Exception:
                # A DB hiccup should not block the explicit creation flow.
                pass
        ...
        try:
            ...
            return _err(rid, 5000, f"genskills session init failed: {exc}")

        message = build_genskills_invocation_message(arg, task_id=new_key)
        notice = (
            f"开始整理这个做法：{arg[:80]}{'...' if len(arg) > 80 else ''}"
            if arg.strip()
            else "开始创建一个以后能直接用的做法。"
        )
        return _ok(rid, {"type": "send", "message": message, "notice": notice})
