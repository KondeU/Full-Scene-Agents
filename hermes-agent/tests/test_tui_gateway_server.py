...
def test_genskills_dispatch_restarts_with_skills_only_toolset(monkeypatch):
    captured = {}
    ...
    class FakeDB:
        ...
        def create_session(self, *args, **kwargs):
            captured["created"] = args

        def set_session_title(self, *args, **kwargs):
            captured["title"] = args

    def fake_make_agent(sid, key, session_id=None, enabled_toolsets=None):
        captured["make_agent"] = {
            "sid": sid,
            "key": key,
        ...
        return types.SimpleNamespace(model="test", enabled_toolsets=enabled_toolsets)

    server._sessions["sid"] = _session()
    monkeypatch.setattr(server, "_new_session_key", lambda: "genskills-key")
    monkeypatch.setattr(server, "_get_db", lambda: FakeDB())
    ...
    monkeypatch.setattr(
        server,
        "_session_info",
        lambda agent, session: {"model": "test", "enabled_toolsets": agent.enabled_toolsets},
    )
        ...
        resp = server.handle_request(
            {
                "id": "1",
                "method": "command.dispatch",
                "params": {"name": "genskills", "arg": "健康周报", "session_id": "sid"},
            }
        )
    finally:
        server._sessions.pop("sid", None)
    ...
    assert captured["make_agent"]["enabled_toolsets"] == ["skills"]
    assert server._sessions.get("sid", {}).get("session_key") != "session-key"
