...
class TestCapabilitiesEndpoint:
    ...
    async def test_capabilities_advertises_plugin_safe_contract(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            assert data["features"]["skill_launchers_api"] is True
            assert data["features"]["session_continuity_header"] == "X-Hermes-Session-Id"
            assert data["skill_launchers"][0]["id"] == "genskills"
            assert data["skill_launchers"][0]["prefix"] == "/genskills"
...
class TestChatCompletionsEndpoint:
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("command", ["/genskills", "/genSkills", "／genSkills"])
    async def test_genskills_slash_rewrites_to_invocation_and_enables_skills(self, adapter, command):
        app = _create_app(adapter)
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    ...
                    json={
                        "model": "hermes-agent",
                        "messages": [{"role": "user", "content": f"{command} 健康周报"}],
                    },
                )

        assert resp.status == 200
        kwargs = mock_run.call_args.kwargs
        assert "The user invoked /genskills" in kwargs["user_message"]
        assert "健康周报" in kwargs["user_message"]
        assert kwargs["extra_toolsets"] == {"skills"}
        assert kwargs["force_toolsets"] == {"skills"}

    @pytest.mark.asyncio
    async def test_plain_chat_does_not_force_skills_toolset(self, adapter):
        app = _create_app(adapter)
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                mock_run.return_value = (
        ...
        assert kwargs["extra_toolsets"] is None
        assert kwargs["force_toolsets"] is None

    @pytest.mark.asyncio
    async def test_legacy_genskills_prompt_rewrites_to_server_invocation(self, adapter):
        legacy_prompt = "\n".join(
            [
                "[IMPORTANT: The user invoked /genskills. Start a fresh flow for turning one recurring task into a reusable way of working.]",
                "[Title: 创建可复用做法]",
                "",
                "你正在帮助用户把一件常做的事保存成以后能直接复用的做法。",
                "每轮只问一个短问题；如果适合选择题，给出 2 到 4 个编号选项。",
                "",
                "用户想保存的做法：健康周报",
                "请直接围绕这个做法开始澄清，不要再问用户想保存哪件事。",
            ]
        )
        app = _create_app(adapter)
        async with TestClient(TestServer(app)) as cli:
        ...
        assert "The user invoked /genskills" in kwargs["user_message"]
        assert "健康周报" in kwargs["user_message"]
        assert "给出 2 到 4 个编号选项" not in kwargs["user_message"]
        assert kwargs["extra_toolsets"] == {"skills"}
        assert kwargs["force_toolsets"] == {"skills"}
