...
class HermesCLI:
    ...
    def process_command(self, command: str) -> bool:
        ...
        if canonical in {"quit", "exit"}:
            ...
            self.new_session(title=genskills_title(goal))
            msg = build_genskills_invocation_message(goal, task_id=self.session_id)
            if hasattr(self, "_pending_input"):
                self._pending_input.put(msg)
            if goal:
                _cprint(f"  开始整理这个做法：{goal[:80]}{'...' if len(goal) > 80 else ''}")
            else:
                _cprint("  开始创建一个以后能直接用的做法。")
